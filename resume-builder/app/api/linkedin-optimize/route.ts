...
import { generateLinkedInSuggestions } from '@/lib/gemini';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const body = (await request.json()) as LinkedInRequestBody;
    ...
    let suggestions;
    try {
      suggestions = await generateLinkedInSuggestions(
        optimization.optimized_cv_json as OptimizedCv,
        optimization.job_description_raw as string,
      );
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      console.error('LinkedIn AI error:', message);
      return NextResponse.json({ error: 'Failed to generate LinkedIn suggestions' }, { status: 502 });
    }

    await supabase
      .from('optimizations')
      .update({ linkedin_suggestions: suggestions })
      .eq('id', optimizationId)
      .eq('user_id', user.id);
